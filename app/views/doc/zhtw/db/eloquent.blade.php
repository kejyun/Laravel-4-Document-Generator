...
<p>
    就像你已經學過的，實做多對多關係需要中介的資料表，Eloquent 提供了非常好用的方法，幫助我們與中介資料表做互動，舉例來說，假設 <code>User</code> 物件有多個相關的 <code>Role</code> 物件，我們可以在模型中透過存取 <code>pivot</code> 來存取這個關係資料表:
</p>
...
<p>
    這邊會注意到每個 <code>Role</code> 模型會自動被指定一個 <code>pivot</code> 屬性，這個屬性會包含模型的中介資料表，且可能也會被其他的 Eloquent 模型存取。
</p>

<p>
    <code>pivot</code> 屬性在預設的情況下只有包含 "鍵值 (Key)"，如果你想要讓你的 pivot 資料表包含其他的屬性，則必須要在定義模型關係時，去指定要額外存取的屬性:
</p>
...
<p>
    現在 <code>foo</code> 及 <code>bar</code> 屬性可以在 <code>Role</code> 模型中的 <code>pivot</code> 物件中存取了。
</p>

<p>
    如果你想要你的 pivot 資料表可以自動維護 <code>created_at</code> 及 <code>updated_at</code> 這兩個欄位的時間戳記，可以在定義關係時使用 <code>withTimestamps</code> 方法即可:
</p>
...
<p>
    使用 <code>detach</code> 方法可以刪除模型在 pivot 資料表的所有紀錄:
</p>

<p><strong>刪除在 Pivot 資料表的紀錄</strong></p>

<pre><code>User::find(1)-&gt;roles()-&gt;detach();
</code></pre>

<p>
    注意到這個操作不會刪除 <code>roles</code> 資料表的紀錄，僅有移除在 pivot 資料表的紀錄。
</p>
...
<p>
    所有多結果的集合可以經由 Eloquent 的 <code>get</code> 方法或模型關係回傳 Eloquent <code>集合 (Collection)</code> 物件，這個物件實做了 PHP 的 <code>IteratorAggregate</code> 介面，所以可以像陣列一樣遞迴存取，然而物件也有其他各種的方法去存取這個資料集合。
</p>

<p>
    舉例來說，我們可以使用 <code>contains</code> 方法，判斷集合資料中是否有指定的 "主鍵 (primary key)" 資料:
</p>

<p><strong>檢查集合是否包含鍵值資料</strong></p>
...
<p><strong>實做回呼函式到每個集合的物件</strong></p>
...
<p>
    有時你可能會想要在你自己加入的方法中，回傳自訂的集合物件，你可以在 Eloquent 中去定義覆寫 <code>newCollection</code> 的方法:
</p>

<p><strong>回傳自訂集合類型</strong></p>
...
<p>
    Eloquent 提供一個便利的方法讓你可以在"取得"或"設定"屬性時，轉換模型中的屬性，只要簡單的在模型中定義 <code>getFooAttribute</code> 方法當作存取器，這裡需要記住的是方法名稱必須遵循 <code>駝峰式大小寫 (camelCase)</code>，即便你的資料表欄位名稱規則是 <code>蛇底式小寫 (snake_case)</code>:
</p>

<p><strong>定義屬性存取器</strong></p>
...
<p>
    在上述範例中，<code>first_name</code> 欄位會有一個存取器，欄位資料將會傳送給存取器。
</p>

<p>修改器的定義也很類似:</p>

<p><strong>定義修改器</strong></p>
...
<p>
    Eloquent 預設會將 <code>created_at</code> 、 <code>updated_at</code> 及 <code>deleted_at</code> 欄位轉換為 <a href="https://github.com/briannesbitt/Carbon" target="_blank">Carbon</a> 實例，該實例提供了各種好用的處理時間方法，且繼承了 PHP 原生的 <code>DateTime</code> 類別。
</p>

<p>
    你可以在模型 (Model) 中透過 <code>getDates</code> 方法，自訂那些欄位要自動被變異，或是要關閉這些變異:
</p>
...
<p>
    當欄位是日期格式時，你可以將此欄位設定為 UNIX 時間戳記、日期字串 (<code>Y-m-d</code>)，日期時間字串 (<code>Datetime</code>)，且必然是 <code>DateTime</code> / <code>Carbon</code> 的實例。
</p>

<p>
    為了完全的關閉這些欄位變異，只要在 <code>getDates</code> 方法中簡單的回傳空陣列即可:
</p>
...
<p>
    Eloquent 模型會驅動數個事件，允許你藉由下列方法 <code>creating</code> 、 <code>created</code> 、 <code>updating</code> 、 <code>updated</code> 、 <code>saving</code> 、 <code>saved</code> 、 <code>deleting</code> 、 <code>deleted</code>，去取得模型的生命週期中的不同事件點，假如 <code>creating</code> 、 <code>updating</code> 或 <code>saving</code> 事件回傳 <code>false</code>，則原事件的動作將會被取消:
</p>

<p><strong>透過事件取消儲存操作</strong></p>
...
<p>
    Eloquent 模型也包含靜態的 <code>boot</code> 方法，提供一個地方可以放置及註冊你的事件綁定動作。
</p>

<p><strong>設定模型 Boot 方法</strong></p>
...
<p>
    為了保護模型事件的處理，你可以註冊一個模型觀察器，觀察器類別有與模型事件相符的方法，舉例來說觀察器中會有 <code>creating</code> 、 <code>updating</code> 、 <code>saving</code> 的方法，除了這些方法，還有其他模型事件的名稱的方法。
</p>

<p>所以舉例來說，模型觀察器會長得像這樣:</p>
...
<p>
    你可以使用 <code>observe</code> 方法去註冊一個觀察器的實例:
</p>
...
<p>
    當建立 JSON 格式的 API，你會經常需要將模型或關係的資訊轉換成 "陣列" 或 "JSON" 格式，所以 Eloquent 也包含了這些資料格式的轉換方法，你可以使用 <code>toArray</code> 方法，將 "模型 (Model)" 及模型載入的 "關係資料 (Relationship)" 成為陣列格式的資料:
</p>

<p><strong>轉換模型為陣列格式資料</strong></p>
...
<p>
    可以使用 <code>toJson</code> 方法，將模型轉換為 JSON 資料格式:
</p>

<p><strong>轉換模型為 JSON 格式資料</strong></p>
...
<p>
    有時你可能會想要限制模型轉換成 "陣列" 或 "JSON" 格式時的屬性資料有哪些，像是密碼欄位資料不想要轉換輸出，可以在你的模型加入 <code>hidden</code> 屬性，來達到這樣的目的:
</p>

<p><strong>在陣列或JSON資料格式轉換時隱藏屬性欄位</strong></p>
...
<p>
    另外，你也可以使用 <code>visible</code> 屬性去定義可以轉換輸出的白名單屬性欄位:
</p>
